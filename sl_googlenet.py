...
def train_CNN(epochs, save_root, network):
    ...
    for i in range(epochs):  # 训练
        ...
        with torch.no_grad():  # batch测试
            for b, (X_test, y_test) in enumerate(test_loader):
                ...
                # if b % 50 == 0:  # 每50个batch打印一次损失和正确率
                #     print(f'test accuracy:{tst_corr.item() * 100 / (10 * (b+1)):7.3f}%','test_corr:',tst_corr)
    ...
    print(f'test accuracy:{my_tst_corr * 100 / 306:7.3f}%')


    # total = 0
    # for ele in test_correct:
    #     total += test_correct[ele]
    # print(f'test accuracy:{total/300:7.3f}%')
